...
kubectl get flavor -n fluidos --no-headers --kubeconfig $PWD/provider-IT-config.yaml | cut -f1 -d\  | xargs -I% kubectl patch flavor/%  --patch-file ../../tests/examples/carbon-bad-patch-file.yaml --type merge -n fluidos --kubeconfig $PWD/provider-IT-config.yaml

# configure carbon emission in DE (good)
kubectl get flavor -n fluidos --no-headers --kubeconfig $PWD/provider-DE-config.yaml | cut -f1 -d\  | xargs -I% kubectl patch flavor/%  --patch-file ../../tests/examples/carbon-good-patch-file.yaml --type merge -n fluidos --kubeconfig $PWD/provider-DE-config.yaml
